...
class IndexedRowMatrix(DistributedMatrix):
    ...
    def rows(self):
        ...
        return rows_df.map(lambda row: IndexedRow(row[0], row[1]))
...
class CoordinateMatrix(DistributedMatrix):
    ...
    def entries(self):
        ...
        return entries_df.map(lambda row: MatrixEntry(row[0], row[1], row[2]))
...
def _convert_to_matrix_block_tuple(block):
    ...
    return (blockRowIndex, int(block[0][1])), block[1]
...
class BlockMatrix(DistributedMatrix):
    ...
    def blocks(self):
        ...
        return blocks_df.map(lambda row: ((row[0][0], row[0][1]), row[1]))
